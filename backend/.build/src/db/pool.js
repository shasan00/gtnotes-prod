...
function getPool() {
    if (!pool) {
        ...
        const sslConfig = process.env.RDS_CA_BASE64
            ? { ca: buffer_1.Buffer.from(process.env.RDS_CA_BASE64, "base64").toString("utf-8") }
...
async function initDb() {
  ...
  `);
    ...
    await p
        .query(`create unique index if not exists idx_users_microsoft_id on users(microsoft_id) where microsoft_id is not null;`)
        .catch(() => { });
    try {
